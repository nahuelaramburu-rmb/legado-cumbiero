...
export default async function TenantAdminPage({
  ...
  const tenant = db.getTenantWithShows(tenantSlug, {
    withReservations: true,
    withGuests: true,
  });
  if (!tenant) notFound();

  const totalReservas = tenant.shows.reduce(
    (acc, s) => acc + s.reservations.reduce((a, r) => a + r.quantity, 0),
    0
  );
  const totalInvitados = tenant.shows.reduce(
    (acc, s) => acc + s.guestEntries.reduce((a, g) => a + 1 + g.plusOnes, 0),
    0
  );
      ...
      <main className="mx-auto max-w-5xl px-6 py-14">
        ...
        <div className="mb-10 grid gap-4 sm:grid-cols-3">
          <div className="card p-5">
            ...
            <p className="text-3xl font-black text-cumbia-cream">{totalReservas}</p>
          </div>
          <div className="card p-5">
            <p className="text-xs uppercase text-cumbia-cream/50">En listas de invitados</p>
            <p className="text-3xl font-black text-cumbia-cream">{totalInvitados}</p>
          </div>
          <div className="card p-5">
            <p className="text-xs uppercase text-cumbia-cream/50">Shows cargados</p>
            <p className="text-3xl font-black text-cumbia-cream">{tenant.shows.length}</p>
          </div>
        </div>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="card p-5">
                <h4 className="mb-3 text-sm font-semibold text-cumbia-cream/80">
                  Reservas / entradas ({show.reservations.length})
                </h4>
                <div className="space-y-2">
                  {show.reservations.length === 0 && (
                    <p className="text-sm text-cumbia-cream/40">Sin reservas todavía</p>
                  )}
                  {show.reservations.map((r) => (
                    <div
                      key={r.id}
              ...
              <div className="card p-5">
                <h4 className="mb-3 text-sm font-semibold text-cumbia-cream/80">
                  Lista de invitados ({show.guestEntries.length})
                </h4>
                <form
                ...
                <div className="space-y-2">
                  {show.guestEntries.length === 0 && (
                    <p className="text-sm text-cumbia-cream/40">Lista vacía</p>
                  )}
                  {show.guestEntries.map((g) => (
                    <div
                      key={g.id}
                      className="flex items-center justify-between rounded-lg bg-black/20 px-3 py-2 text-sm"
                    >
